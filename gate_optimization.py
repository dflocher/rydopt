...
# optmimization of a single pulse starting from given initial parameters. Called in main of this file
def train_single_gate(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, params, N_epochs, learning_rate, T_penalty, decay):
    Hamiltonians, input_states, fidelity_fn = hamiltonians.get_subsystem_Hamiltonians(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, decay)
    optimizer = optax.adam(learning_rate=learning_rate)
    opt_state = optimizer.init(params)
    start = time.time()
    for i in range(N_epochs):
        params, opt_state, loss_value = opt_step(params, opt_state, optimizer, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty)
        if i % 10 == 0:
    ...
    final_loss = loss_fn(params, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty)
...
# optimization of multiple pulses from random initial parameters. Called in main of this file
def gate_search(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, T_default, N_searches, N_params, N_epochs, learning_rate, T_penalty, decay):
    Hamiltonians, input_states, fidelity_fn = hamiltonians.get_subsystem_Hamiltonians(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, decay)
    optimizer = optax.adam(learning_rate=learning_rate)
    ...
    for j in range(N_searches):
        ...
        params = jnp.array([T_default + 0.3 * T_default * r[0]] + [r[i] for i in range(1, N_params)])
        opt_state = optimizer.init(params)
        loss_value = 0.0
        for i in range(N_epochs):
            params, opt_state, loss_value = opt_step(params, opt_state, optimizer, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty)
        result_str = 'search: {search:d}, loss: {loss:.6f}, fidelity: {fid:.6f}'.format(search=j, loss=loss_value, fid=loss_value-T_penalty*params[0])
        if loss_value - T_penalty * params[0] <= -0.999:
            params_str = ', params: [' + ', '.join('{p:.5f}'.format(p=p) for p in params) + ']'
        ...
        if loss_value < best_loss:
            ...
            best_params = params
            best_index = j
    print('\nBest run: {r:d}, cost: {c:.6f}, T: {T:.4f}'.format(r=best_index, c=best_loss, T=best_params[0]))
    return best_params
...
def gate_search_cluster(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, T_default, N_searches, N_params, N_epochs, learning_rate, T_penalty, decay):
    Hamiltonians, input_states, fidelity_fn = hamiltonians.get_subsystem_Hamiltonians(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, decay)
    optimizer = optax.adam(learning_rate=learning_rate)
    key = jax.random.PRNGKey(time.time_ns())
    ...
    start = 0.0
    for j in range(N_searches):
        if j == 1:  # start measuring the runtime from the 2nd search to avoid measuring the compile time
            start = time.time()
        key, subkey = jax.random.split(key)
        r = jax.random.normal(subkey, (N_params,))
        params = jnp.array([T_default + 0.3 * T_default * r[0]] + [r[i] for i in range(1, N_params)])
...
@partial(jax.jit, static_argnames=['optimizer', 'pulse', 'Hamiltonians', 'fidelity_fn', 'T_penalty'])
def opt_step(params, opt_state, optimizer, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty):
    loss_value, grads = jax.value_and_grad(loss_fn, argnums=0)(params, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty)
...
def loss_fn(params, pulse, Hamiltonians, input_states, fidelity_fn, T_penalty):
    ...
    for H, psi_in in zip(Hamiltonians, input_states):
        ...
        psi_out = propagate(psi_in, schroedinger, args)
...
def propagate(psi_initial, schroedinger_eq, args):
    times = args[0]
    T = times[-1]
    ...
    sol = diffrax.diffeqsolve(term, solver, t0=0.0, t1=T, dt0=None, y0=psi_initial, args=args,
...
if __name__ == '__main__':
    ...
    params = train_single_gate(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, initial_params, N_epochs, learning_rate, T_penalty, decay)
    ...
    pulse_visualization.visualize_subsystem_dynamics(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse.__name__, params, decay)

    # verify the pulse using the full Hamiltonian (using qutip)
    pulse_verification.verify(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse.__name__, params, decay)
