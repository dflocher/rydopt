...
if __name__ == '__main__':
    ...
    all_costs, all_params, runtime = gate_optimization.gate_search_cluster(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse,
                                                                           T_default, N_searches, N_params, N_epochs, learning_rate, T_penalty, decay)

    directory = os.path.dirname(__file__)
    file = (directory + '/../Data/data_' + n_atoms_str + '_atoms' + '__theta_' + theta_str + '__eps_' + eps_str + '__lamb_' + lamb_str +
            '__delta_' + delta_str + '__kappa_' + kappa_str + '__Vnn_' + Vnn_str + '__Vnnn_' + Vnnn_str + '__' + pulse_str +
            '__params_' + N_params_str + decay_on_off + '__run_' + run_str + '.npz')

    np.savez(file, n_atoms=n_atoms, Vnn=Vnn, Vnnn=Vnnn, theta=theta, eps=eps, lamb=lamb, delta=delta, kappa=kappa, pulse=pulse_str,
             T_default=T_default, N_searches=N_searches, N_params=N_params, N_epochs=N_epochs, learning_rate=learning_rate, T_penalty=T_penalty,
             decay=decay, run=run, runtime=runtime, all_costs=all_costs, all_params=all_params)
