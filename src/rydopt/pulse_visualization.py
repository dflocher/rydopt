...
def get_subsystem_Hs(n_atoms, Vnn, Vnnn, Delta_of_t, Phi_of_t, decay):
    def H_2LS_1(t):
        return (qt.basis(2, 1).proj() * (Delta_of_t(t) - 1j * 0.5 * decay) +
    ...
    def H_3LS_Vnnn(t):
        return (qt.basis(3, 2).proj() * Vnnn +
                qt.basis(3, 1).proj() * (Delta_of_t(t) - 1j * 0.5 * decay) +
                qt.basis(3, 2).proj() * 2 * (Delta_of_t(t) - 1j * 0.5 * decay) +
                0.5 * np.sqrt(2) * (I0x1I_3LS + I1x0I_3LS + I1x2I_3LS + I2x1I_3LS) * np.cos(Phi_of_t(t)) +
                0.5 * np.sqrt(2) * (-1j*I0x1I_3LS + 1j*I1x0I_3LS - 1j*I1x2I_3LS + 1j*I2x1I_3LS) * np.sin(Phi_of_t(t)))

    def H_3LS_Vnn(t):
        return (qt.basis(3, 2).proj() * Vnn +
                qt.basis(3, 1).proj() * (Delta_of_t(t) - 1j * 0.5 * decay) +
                qt.basis(3, 2).proj() * 2 * (Delta_of_t(t) - 1j * 0.5 * decay) +
                0.5 * np.sqrt(2) * (I0x1I_3LS + I1x0I_3LS + I1x2I_3LS + I2x1I_3LS) * np.cos(Phi_of_t(t)) +
                0.5 * np.sqrt(2) * (-1j*I0x1I_3LS + 1j*I1x0I_3LS - 1j*I1x2I_3LS + 1j*I2x1I_3LS) * np.sin(Phi_of_t(t)))
    ...
    def H_4LS(t):
        return (qt.basis(4, 3).proj() * Vnnn +
                ...
                0.5 * np.sqrt(3) * (-1j*I0x1I_4LS + 1j*I1x0I_4LS) * np.sin(Phi_of_t(t)) +
                (1/np.sqrt(3)) * (I1x3I_4LS + I3x1I_4LS) * np.cos(Phi_of_t(t)) +
                (1/np.sqrt(3)) * (-1j*I1x3I_4LS + 1j*I3x1I_4LS) * np.sin(Phi_of_t(t)) +
                (1/np.sqrt(6)) * (I2x3I_4LS + I3x2I_4LS) * np.cos(Phi_of_t(t)) +
                (1/np.sqrt(6)) * (-1j*I2x3I_4LS + 1j*I3x2I_4LS) * np.sin(Phi_of_t(t)))

    def H_6LS(t):
        Omega = np.cos(Phi_of_t(t)) + 1j * np.sin(Phi_of_t(t))
        H_6LS_upper_diagonal = (0.5 * np.sqrt(3) * np.conj(Omega) * I0x1I_6LS + np.conj(Omega) * I1x4I_6LS - 0.5 * np.conj(Omega) * I2x3I_6LS +
                                (1 / 3) * np.sqrt(2) * (Vnn - Vnnn) * I3x4I_6LS + 0.5 * np.sqrt(3) * np.conj(Omega) * I4x5I_6LS)
        H_6LS_diagonal = (((1 / 3) * Vnn + (2 / 3) * Vnnn) * qt.basis(6, 3).proj() +
    ...
    def H_4LS_Vnnn(t):
        ...
        H_4LS_V_upper_diagonal = (0.5 * np.sqrt(3) * Omega_c * I0x1I_4LS + Omega_c * I1x2I_4LS + 0.5 * np.sqrt(3) * Omega_c * I2x3I_4LS)
        H_4LS_V_diagonal = (Vnnn * qt.basis(4, 2).proj() +
    ...
    def H_5LS(t):
        Omega_c = np.cos(Phi_of_t(t)) - 1j * np.sin(Phi_of_t(t))
        H_5LS_upper_diagonal = (Omega_c * I0x1I_5LS +
                                0.5 * np.sqrt(3) * Omega_c * I1x3I_5LS +
                                0.5 * Omega_c * I2x3I_5LS +
                                0.5 * np.sqrt(3) * Omega_c * I3x4I_5LS)
        H_5LS_diagonal = ((Delta_of_t(t) - 1j * 0.5 * decay) * (qt.basis(5, 1).proj() + qt.basis(5, 2).proj()) +
                          (2 * (Delta_of_t(t) - 1j * 0.5 * decay) + Vnnn) * qt.basis(5, 3).proj() +
                          (3 * (Delta_of_t(t) - 1j * 0.5 * decay) + 3 * Vnnn) * qt.basis(5, 4).proj())
        return H_5LS_upper_diagonal + H_5LS_upper_diagonal.dag() + H_5LS_diagonal

    def H_8LS(t):
        Omega_c = np.cos(Phi_of_t(t)) - 1j * np.sin(Phi_of_t(t))
        H_8LS_upper_diagonal = (Omega_c * I0x1I_8LS +
        ...
        H_8LS_diagonal = ((Delta_of_t(t) - 1j * 0.5 * decay) * (qt.basis(8, 1).proj() + qt.basis(8, 2).proj()) +
                          (2 * (Delta_of_t(t) - 1j * 0.5 * decay) + Vnn) * qt.basis(8, 3).proj() +
                          (2 * (Delta_of_t(t) - 1j * 0.5 * decay) + Vnnn) * qt.basis(8, 4).proj() +
                          (3 * (Delta_of_t(t) - 1j * 0.5 * decay) + 2 * Vnn + Vnnn) * qt.basis(8, 5).proj() +
                          (3 * (Delta_of_t(t) - 1j * 0.5 * decay) + 3 * Vnnn) * qt.basis(8, 6).proj() +
                          (4 * (Delta_of_t(t) - 1j * 0.5 * decay) + 3 * Vnn + 3 * Vnnn) * qt.basis(8, 7).proj())
    ...
    if n_atoms == 2 and Vnn == float("inf"):
        ...
        return [(H_2LS_1, 2), (H_2LS_sqrt2, 2), (H_3LS_Vnnn, 3), (H_4LS, 4), (H_4LS_Vnnn, 4), (H_5LS, 5)]
    elif n_atoms == 4:
        return [(H_2LS_1, 2), (H_3LS_Vnn, 3), (H_3LS_Vnnn, 3), (H_6LS, 6), (H_4LS_Vnnn, 4), (H_8LS, 8)]
    else:
...
def time_evolution(H, dim, T, decay):
    ...
    result = qt.mesolve(H, psi_in, t_list, e_ops=e_ops,
                        options={'store_final_state': True, 'normalize_output': normalize, 'atol': 1e-30, 'rtol': 1e-15})
...
def visualize_subsystem_dynamics(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, params, decay, plot=True):
    if isinstance(pulse, str):
        pulse = pulses_qutip.get_pulse(pulse)
    params = np.array(params)
    T = params[0]
    ...
    _, _, fidelity_fn = hamiltonians.get_subsystem_Hamiltonians(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, decay)
...
if __name__ == '__main__':
    ...
    visualize_subsystem_dynamics(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, params, decay)
