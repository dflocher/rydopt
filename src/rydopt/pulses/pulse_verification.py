...
def get_H(n_atoms, Vnn, Vnnn, Delta_of_t, Phi_of_t, decay):
    def H_2atoms(t):
        return (
            H2_Vnn * Vnn
            + H2_Delta * (Delta_of_t(t) - 1j * 0.5 * decay)
            + H2_Omega_real * np.cos(Phi_of_t(t))
            + H2_Omega_imag * np.sin(Phi_of_t(t))
        )
    ...
    def H_3atoms(t):
        return (
            H3_Vnnn * Vnnn
            + H3_Vnn * Vnn
            + H3_Delta * (Delta_of_t(t) - 1j * 0.5 * decay)
            + H3_Omega_real * np.cos(Phi_of_t(t))
            + H3_Omega_imag * np.sin(Phi_of_t(t))
        )
    ...
    def H_4atoms(t):
        return (
            H4_Vnnn * Vnnn
            + H4_Vnn * Vnn
            + H4_Delta * (Delta_of_t(t) - 1j * 0.5 * decay)
            + H4_Omega_real * np.cos(Phi_of_t(t))
            + H4_Omega_imag * np.sin(Phi_of_t(t))
        )
...
def apply_pulse_and_compare(n_atoms, H, T, theta, eps, lamb, delta, kappa, decay):
    ...
    if n_atoms == 2:
        ...
        F = (
            qt.fidelity(psi_out, psi_target) ** 2
        )  # qutip.fidelity is defined like in Nielsen & Chuang
    elif n_atoms == 3:
...
def construct_target_state_2atoms(phi, theta):
    ...
    entangling_gate = qt.tensor(id3, id3) + (np.exp(1j * theta) - 1) * qt.tensor(
        I1x1I, I1x1I
    )
    plus_state = (qt.basis(3, 0) + qt.basis(3, 1)).unit()
...
def construct_target_state_3atoms(phi, theta, eps, lamb):
    ...
    entangling_gate = (
        ...
        + (np.exp(1j * lamb + 2j * theta + 1j * eps) - 1)
...
def construct_target_state_4atoms(phi, theta, eps, lamb, delta, kappa):
    ...
    entangling_gate = (
        ...
        * qt.tensor(qt.tensor(qt.tensor(I0x0I + IrxrI, I1x1I), I1x1I), I1x1I)
        + (np.exp(1j * (kappa + delta + 3 * lamb + 3 * theta + 3 * eps)) - 1)
        * qt.tensor(qt.tensor(qt.tensor(I1x1I, I1x1I), I1x1I), I1x1I)
    )
    plus_state = (qt.basis(3, 0) + qt.basis(3, 1)).unit()
    psi_target = qt.tensor(
        qt.tensor(qt.tensor(plus_state, plus_state), plus_state), plus_state
    )
    psi_target = entangling_gate * global_z_rotation * psi_target
    return psi_target
...
def verify(n_atoms, Vnn, Vnnn, theta, eps, lamb, delta, kappa, pulse, params, decay):
    ...
    fidelity, TR = apply_pulse_and_compare(
        n_atoms, H, T, theta, eps, lamb, delta, kappa, decay
    )
